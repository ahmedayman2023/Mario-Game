...
export default function Layout({ children, currentPageName }: { children: React.ReactNode, currentPageName: string }) {
  ...
  return (
    <div className="dark min-h-screen relative">
      ...
      <main className={`max-w-5xl mx-auto px-4 sm:px-6 py-10 ${['Timer', 'SRS'].includes(currentPageName) ? 'bg-black text-white rounded-none ring-0 shadow-none' : 'text-slate-100 rounded-3xl ring-1 ring-emerald-500/15 bg-slate-900/30 shadow-[0_0_40px_-10px_rgba(16,185,129,0.35)]'}`}>
        {children}
      </main>
