  ...
  return (
    <div className="relative w-32 h-20 sm:w-40 sm:h-24 flex flex-col items-center justify-end overflow-visible">
      <svg viewBox="0 0 200 120" className="absolute top-0 left-0 w-full h-full overflow-visible">
        ...
        {/* Track Background */}
        <path d="M 20 100 A 80 80 0 0 1 180 100" fill="none" stroke="rgba(255,255,255,0.15)" strokeWidth="12" strokeLinecap="round" />
        {/* Colored Track */}
        <path
        ...
        <g transform={`translate(100, 100) rotate(${angle})`} className="transition-transform duration-1000 ease-out">
          <polygon points="-3,0 3,0 0,-75" fill="#e8f1ff" />
          <circle cx="0" cy="0" r="6" fill="#e8f1ff" />
          <circle cx="0" cy="0" r="3" fill="#fbd000" />
        </g>
      </svg>

      <div className="z-10 flex flex-col items-center mt-8 sm:mt-10">
        <span className="text-lg sm:text-xl font-pixel text-white leading-none">{clampedPercentage}%</span>
        <span className="text-[8px] sm:text-[10px] text-white/50 font-bold mt-1">الوقود</span>
      </div>
  ...
  const renderTable = (
    ...
    return (
      <div className="mb-8" key={categoryKey}>
        ...
        <div className="bg-panel mario-block-sm overflow-x-auto">
          <table className="w-full text-right border-collapse min-w-[800px]">
                      ...
                      <>
                        ...
                        <td className="p-2">
                          <input
                            ...
                            className="w-full bg-panel-soft border-2 border-white/20 px-3 py-2 text-sm text-white focus:outline-none focus:border-mario-sky"
                            autoFocus
                          />
                        </td>
                        <td className="p-2">
                      ...
                      <>
                        <td className="px-3 py-2.5 text-center">
                          <button
                            ...
                            className={`w-5 h-5 sm:w-6 sm:h-6 mx-auto flex items-center justify-center border-2 border-white/30 transition-colors ${item.isCompleted ? 'bg-mario-emerald text-white' : 'bg-panel-soft text-transparent hover:bg-mario-emerald/10'}`}
                          >
                            <Check size={14} className={item.isCompleted ? 'opacity-100' : 'opacity-0'} />
                          </button>
                        </td>
                        <td className={`px-3 py-2.5 text-white font-bold text-xs sm:text-sm transition-all ${item.isCompleted ? 'line-through text-white/40' : ''}`}>{item.product}</td>
                        <td className={`px-3 py-2.5 text-white/70 text-xs sm:text-sm transition-all ${item.isCompleted ? 'opacity-50' : ''}`}>{item.goal || '-'}</td>
                        <td className={`px-3 py-2.5 text-white/70 text-xs sm:text-sm transition-all ${item.isCompleted ? 'opacity-50' : ''}`}>
                          <span className="text-mario-red font-black mr-1">{item.cost}</span>
                        </td>
                        <td className={`px-3 py-2.5 text-white/70 text-xs sm:text-sm transition-all ${item.isCompleted ? 'opacity-50' : ''}`}>{item.quantity}</td>
                        <td className={`px-3 py-2.5 text-white/70 text-xs sm:text-sm transition-all ${item.isCompleted ? 'opacity-50' : ''}`}>{item.unit || '-'}</td>
                        <td className={`px-3 py-2.5 text-white/70 text-xs sm:text-sm transition-all ${item.isCompleted ? 'opacity-50' : ''}`} dir="ltr">{item.time || '--:--'}</td>
                        {macroField && (
                          <td className={`px-3 py-2.5 text-white/70 text-xs sm:text-sm transition-all ${item.isCompleted ? 'opacity-50' : ''}`}>{item[macroField] || '-'}</td>
                        )}
                        <td className="px-3 py-2.5">
                          <div className="flex items-center justify-center gap-1 sm:gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              ...
                              className="text-white/50 hover:text-mario-sky p-1.5 transition-colors"
                            >
                              <Pencil size={14} />
                            </button>
                            <button
                              onClick={() => deleteItem(item.id)}
                              className="text-white/50 hover:text-mario-red p-1.5 transition-colors"
                            >
                              <Trash2 size={14} />
                            </button>
                          </div>
            ...
            <tfoot className="bg-white/5 border-t-2 border-white/20">
              <tr>
                <td colSpan={3} className="p-4 text-white font-black text-left">الإجمالي:</td>
                ...
                {macroField && (
                  <td className={`p-4 font-black text-lg ${colorClass}`}>
                    {formattedTotalMacro} <span className="text-xs text-white/50 font-normal">جم</span>
                  </td>
                )}
                <td colSpan={1}></td>
              </tr>
  ...
  return (
    ...
    >
      ...
      <div className="mb-6 bg-black mario-block p-4 sm:p-6 flex items-center justify-between">
        ...
        <div className="flex items-center justify-center bg-panel rounded-full mario-block-sm">
      ...
      <div className="flex overflow-x-auto gap-2 mb-4 pb-2 hide-scrollbar">
        {daysOfWeek.map(day => (
          <button
            ...
            className={`mario-btn px-4 py-2 font-black whitespace-nowrap ${selectedDay === day ? 'bg-mario-yellow text-black' : 'bg-panel text-white'}`}
          >
            {day}
          </button>
        ))}
      ...
      {/* Add Item Form */}
      <form onSubmit={addItem} className="mb-6 bg-panel mario-block-sm p-3 flex flex-col sm:flex-row flex-wrap gap-2">
        <input
          type="text"
          value={newProduct}
          onChange={(e) => setNewProduct(e.target.value)}
          placeholder="المنتج (مثال: عصير رمان)"
          className="flex-1 min-w-[120px] bg-panel-soft border-2 border-white/20 px-3 py-2 text-xs sm:text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-mario-sky transition-all font-medium"
        />
        <input
          type="text"
          value={newGoal}
          ...
          placeholder="الهدف"
          className="w-full sm:w-24 bg-panel-soft border-2 border-white/20 px-3 py-2 text-xs sm:text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-mario-sky transition-all font-medium"
        />
        <input
          type="number"
          value={newCost}
        ...
        <select
          ...
          className="w-full sm:w-24 bg-panel-soft border-2 border-white/20 px-3 py-2 text-xs sm:text-sm text-white focus:outline-none focus:border-mario-sky transition-all font-medium"
        >
        ...
        <select
          ...
          className="w-full sm:w-32 bg-panel-soft border-2 border-white/20 px-3 py-2 text-xs sm:text-sm text-white focus:outline-none focus:border-mario-sky transition-all font-medium"
        >
        ...
        {newCategory !== 'general' && (
          <input
            ...
            className="w-full sm:w-32 bg-panel-soft border-2 border-white/20 px-3 py-2 text-xs sm:text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-mario-sky transition-all font-medium"
          />
        )}
        <button
          type="submit"
      ...
      {/* Summary Section */}
      <div className="flex justify-between items-center mb-6 bg-panel mario-block-sm p-4">
        <h2 className="text-base sm:text-lg font-black text-white">إجمالي التكلفة / الكمية لليوم ({selectedDay})</h2>
        <div className="text-mario-red font-pixel text-xl sm:text-2xl">{totalCost}</div>
      </div>

      {/* Tables */}
      {filteredItems.length === 0 ? (
        <div className="p-8 text-center text-white/40 font-bold bg-panel mario-block-sm border-dashed">
        ...
        <>
          ...
          {renderTable('general', '📋 عناصر أخرى', 'text-white/60', '', null)}
        </>
      )}

      {/* Meals Section */}
      <div className="mt-12 mb-8">
        <h2 className="text-xl sm:text-2xl font-black text-white mb-6 flex items-center gap-2 scoreboard-font">
          <Activity className="text-mario-sky" size={24} />
          الوجبات المجمعة
        </h2>

        {/* Create Meal Form */}
        <form onSubmit={addMeal} className="mb-6 bg-panel mario-block-sm p-4">
          <h3 className="text-sm font-black text-white/70 mb-3">تكوين وجبة جديدة من عناصر اليوم ({selectedDay})</h3>
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            <input
              ...
              className="flex-1 bg-panel-soft border-2 border-white/20 px-3 py-2 text-sm text-white focus:outline-none focus:border-mario-sky transition-all font-medium"
            />
            <button
              type="submit"
          ...
          <div className="bg-white/5 border-2 border-white/10 p-3 max-h-48 overflow-y-auto">
            {filteredItems.length === 0 ? (
              <p className="text-white/40 text-sm text-center py-2 font-bold">لا توجد عناصر متاحة في هذا اليوم لتكوين وجبة.</p>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                {filteredItems.map(item => (
                  <label key={item.id} className="flex items-center gap-2 p-2 hover:bg-mario-sky/10 cursor-pointer transition-colors border border-transparent hover:border-white/10">
                    <input
                      type="checkbox"
                      checked={selectedMealItems.includes(item.id)}
                      onChange={() => toggleMealItemSelection(item.id)}
                      className="w-4 h-4 border-white/30 bg-panel-soft text-mario-emerald focus:ring-mario-sky/50 focus:ring-offset-0"
                    />
                    <span className="text-sm text-white truncate flex-1">{item.product}</span>
                    <span className="text-xs text-white/50 bg-white/5 px-1.5 py-0.5 rounded">{item.category === 'protein' ? 'بروتين' : item.category === 'fiber' ? 'ألياف' : item.category === 'complexCarbs' ? 'كربوهيدرات' : item.category === 'sugarLevel' ? 'سكر' : 'عام'}</span>
                  </label>
                ))}
              </div>
            )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {meals.filter(m => m.dayOfWeek === selectedDay).length === 0 ? (
            <div className="col-span-full p-8 text-center text-white/40 font-bold bg-panel mario-block-sm border-dashed">
            ...
            meals.filter(m => m.dayOfWeek === selectedDay).map(meal => {
              ...
              return (
                <div key={meal.id} className="bg-panel mario-block-sm p-4 flex flex-col relative group">
                  <button
                    onClick={() => deleteMeal(meal.id)}
                    className="absolute top-3 left-3 text-white/40 hover:text-mario-red p-1.5 transition-colors opacity-0 group-hover:opacity-100"
                  >
                    <Trash2 size={16} />
                  </button>

                  <h3 className="text-lg font-black text-white mb-3 pr-2 border-r-4 border-mario-yellow">{meal.name}</h3>

                  <div className="flex-1 bg-white/5 p-3 mb-4 border-2 border-white/10">
                    <ul className="space-y-2">
                      ...
                      {mealItems.length === 0 && <li className="text-white/40 text-sm italic">تم حذف جميع عناصر هذه الوجبة.</li>}
                    </ul>
                  </div>

                  <div className="grid grid-cols-5 gap-2 text-center bg-white/5 border-2 border-white/10 p-2">
                    <div className="flex flex-col">
                      <span className="text-[10px] text-white/50">التكلفة</span>
                      <span className="text-sm font-black text-mario-red">{mealCost}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] text-white/50">بروتين</span>
                      <span className="text-sm font-black text-purple-400">{Math.round(mealProtein * 10)/10}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] text-white/50">ألياف</span>
                      <span className="text-sm font-black text-mario-emerald">{Math.round(mealFiber * 10)/10}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] text-white/50">كربوهيدرات</span>
                      <span className="text-sm font-black text-mario-sky">{Math.round(mealCarbs * 10)/10}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] text-white/50">سكر</span>
